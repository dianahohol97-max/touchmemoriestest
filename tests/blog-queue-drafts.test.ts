...
/**
 * Статті для клієнта і статті для партнера звертаються по-різному, і це не
 * недогляд. Бренд-гайд вимагає «ти» — друг, а не магазин, — і саме так написана
 * затверджена `podarunok-vchyteltsi`. Партнерські ж тексти звертаються до
 * агенції або до блогера як до сторони угоди, і живі лендінги
 * (`locales/uk.json`, ключ `partners`) стоять на «ви». Тому список нижче
 * розділений навмисно.
 */
const AUDIENCE: Record<string, 'клієнт' | 'партнер'> = {
    'shcho-podaruvaty-pislia-podorozhi': 'клієнт',
    'iak-blogeru-monetyzuvaty-travel-kontent': 'партнер',
    'iak-zberehty-spohady-z-podorozhi': 'клієнт',
    'partnerska-programa-dlya-turagentstv-loialnist': 'партнер',
    'iak-blogeru-doluchytysia-do-partnerskoi-programy': 'партнер',
    'biudzhetni-podorozhi-2026': 'клієнт',
    'sertyfikat-chy-travelbook-turagentstvu': 'партнер',
    'podarunok-kliientu-pislia-turu-tsyfry': 'партнер',
};

/**
 * Звертання на «ви» — займенник або дієслово другої особи множини.
 *
 * Межа слова тут НЕ через `\b`: у JavaScript `\w` — це латиниця, цифри й
 * підкреслення, тож між пробілом і «в» жодної межі немає і вираз не знаходить
 * нічого взагалі. Перша версія цієї перевірки саме так і мовчала.
 */
const FORMAL = /(?<!\p{L})(ви|вас|вам|вами|ваш\p{L}*|знаєте|хочете|можете|маєте|вважаєте|вирішуєте|пам'ятаєте|напишіть|візьміть|спробуйте)(?!\p{L})/giu;

describe.each(QUEUE)('бренд-редактура %s', slug => {
    const md = draft(slug);

    it('не вживає заборонених бренд-гайдом зворотів', () => {
        const forbidden = [
            'у сучасному світі', 'у наш час', "невід'ємна частина", 'ключовий аспект',
            'якісна продукція', 'широкий асортимент', 'замовляйте вже зараз',
            'не пропустіть', 'дозвольте поділитися', 'варто зазначити',
            'унікальний у своєму роді', 'не має аналогів', 'ласкаво просимо',
        ];
        for (const phrase of forbidden) {
            expect(md.toLowerCase().includes(phrase), `${slug}: «${phrase}»`).toBe(false);
        }
    });

    it('обходиться без емодзі й без подвійних окликів', () => {
        expect(md).not.toMatch(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}]/u);
        expect(md).not.toMatch(/!{2,}/);
    });

    it('звертається так, як належить своїй аудиторії', () => {
        const formal = md.match(FORMAL) || [];
        if (AUDIENCE[slug] === 'клієнт') {
            // «ви» в сенсі «ви двоє» тут теж не потрібне: статті для клієнта
            // говорять з однією людиною.
            expect(formal, `${slug}: стаття для клієнта звертається на «ви» — ${formal.join(', ')}`).toEqual([]);
        } else {
            expect(formal.length, `${slug}: партнерська стаття втратила «ви»`).toBeGreaterThan(0);
        }
    });
});
