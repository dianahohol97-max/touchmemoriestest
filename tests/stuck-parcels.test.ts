...
describe('stuckLabel для посилки на відділенні', () => {
    it('називає дату прибуття і скільки лишилось безкоштовного зберігання', () => {
        const [p] = stuckParcels([row({
            tracking_status: 'Прибув на відділення',
            tracking_status_at: ago(3),
        })], NOW).atBranch;
        expect(stuckLabel(p)).toBe(
            'Прибув на відділення, прибула 12.09, безкоштовно до 17.09, лишилось 2 дн., ТТН 20451501599844',
        );
    });

    it('після п’яти днів каже, що зберігання вже платне', () => {
        const [p] = stuckParcels([row({
            tracking_status: 'Прибув на відділення',
