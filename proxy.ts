...
const SKIP_PREFIXES = [
    ...
    '/llms.txt',
    // Те саме повторилося 22.09.2026, слово в слово: разом із блогом приїхали
    // дві кореневі адреси, і жодна не підпадала під наявні префікси.
    // `/sitemap-index.xml` урятувався випадково — він починається на
    // `/sitemap`, — а `/blog-sitemap.xml` починається на `/blog`, тож пішов на
    // `/uk/blog-sitemap.xml` і віддав 404 роботам, яким його назвав
    // `robots.txt`. Помилка мовчазна: сторінка є, код є, адреса відповідає
    // кодом 404, і побачити це можна лише запитом ззовні.
    //
    // На це тепер стоїть tests/proxy-root-routes.test.ts: він перебирає
    // кореневі маршрути в `app/` і падає, якщо черговий не потрапив сюди.
    '/blog-sitemap.xml',
    '/indexnow-key.txt',
];
