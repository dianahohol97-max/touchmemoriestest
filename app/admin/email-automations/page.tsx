                            ...
                            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                                <div style={{ flex: '0 0 220px' }}>
                                    <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#64748b', marginBottom: 4 }}>
                                        Промокод {row.key === 'birthday' ? '(генерується автоматично)' : ''}
                                    </label>
                                    <input
                                        value={row.promo_code || ''}
                                        onChange={e => update(row.key, { promo_code: e.target.value })}
                                        disabled={row.key === 'birthday'}
                                        placeholder={row.key === 'birthday' ? 'персональний код' : '—'}
                                        style={{ width: '100%', boxSizing: 'border-box', border: '1px solid #cbd5e1', borderRadius: 8, padding: '9px 12px', fontSize: 14, background: row.key === 'birthday' ? '#f1f5f9' : '#fff' }}
                                    />
                                </div>
                                <button
                                    onClick={() => save(row)}
