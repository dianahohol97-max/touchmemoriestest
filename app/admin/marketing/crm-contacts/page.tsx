...
export default function CrmContactsPage() {
    ...
    const [stats, setStats] = useState({ matched: 0, total: 0, withOrderDate: 0, pageSize: 50 });
    ...
    const load = useCallback(async (p: number, q: string) => {
        ...
        try {
            ...
            setStats({ matched: data.matched, total: data.total, withOrderDate: data.withOrderDate, pageSize: data.pageSize });
        } catch (e: any) {
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '8px 4px' }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 820 }}>
                    ...
                    <tbody>
                        {loading && (
                            <tr><td style={{ ...td, color: '#94a3b8' }} colSpan={7}>Завантажую…</td></tr>
                        )}
                        {!loading && contacts.length === 0 && (
                            <tr><td style={{ ...td, color: '#94a3b8' }} colSpan={7}>Нічого не знайшлося</td></tr>
                        )}
