...
export function useWeddingPhotos(slug: string) {
   ...
   *
   * ВІДЕОПОБАЖАННЯ СЮДИ НЕ ПОТРАПЛЯЄ, і це не дрібниця. Сервер побажань не
   * віддає взагалі, тож інші гості їх не бачать; але той, хто щойно записав
   * листівку, отримував її назад прямо з відповіді про завантаження — і вона
   * лягала в галерею просто в нього на очах, усупереч тому, що сторінка йому
   * пообіцяла рядком вище кнопки. Трималося воно до першого перезавантаження,
   * бо опитування вміє лише додавати, а не прибирати.
   *
   * Перевірка стоїть саме тут, а не в місці виклику: так її не обійде жоден
   * новий блок завантаження, який колись додадуть поруч.
   */
  const addOwn = useCallback(
    (photo: WeddingPhoto) => {
      if (photo.is_wish) return;
      merge([photo], 'start');
    },
    [merge]
  );
