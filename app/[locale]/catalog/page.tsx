  ...
  try {
    ...
    if (supabase) {
      const [{ data: cats }, { data: prods }] = await Promise.all([
        ...
        supabase.from('products').select('id, name, slug, price, price_from, short_description, images, is_popular, popular_order, category_id, fulfillment_type, translations').eq('is_active', true).order('created_at'),
      ]);
