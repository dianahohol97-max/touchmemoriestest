...
export default function CheckoutPage() {
    ...
    /**
     * Ключ ідемпотентності: один на ЗАМОВЛЕННЯ, стабільний між кліками і між
     * поверненнями «назад» зі сторінки Monobank (bfcache воскрешає цю сторінку
     * з повним кошиком і розблокованою кнопкою — саме так один кошик став
     * трьома замовленнями TM-001215/16/17). Сервер за цим ключем повертає вже
     * створене замовлення замість нового рядка.
     *
     * Ключ рахувався ЛИШЕ з вмісту кошика — [id, qty, price]. Але замовлення
     * визначає не лише кошик: у payload їдуть контакти, спосіб і адреса
     * доставки, тип оплати, промокод, сертифікат і бонуси. Жодне з цих полів у
     * ключ не входило, тож сценарій із того самого коментаря вище ламався:
     * клієнтка тисне «назад» із Monobank, виправляє відділення Нової пошти або
     * телефон, тисне «Оформити» ще раз — кошик той самий, ключ той самий,
     * сервер повертає ПЕРШЕ замовлення, і виправлення просто зникає. Посилку
     * везуть на старе відділення.
     *
     * Тепер у підпис входить усе, що описує замовлення. Подвійний клік ці поля
     * не змінює, тож захист від дублів лишається; свідома правка змінює — і
     * тоді нове замовлення саме те, що треба.
     *
     * Свідомо НЕ входять похідні, які довантажуються асинхронно
     * (bonusRedeemed, certApplied, promoDiscount, курс із /api/exchange-rate):
     * якщо така величина приїде між двома кліками, ключ зміниться сам собою і
     * ми отримаємо дубль. Тому в підписі стоїть вибір користувача (useBonus,
     * promoId, certCode), а не порахована з нього сума.
     */
    const submitKeyFor = (cartItems: any[]): string => {
        const sig = JSON.stringify({
            items: cartItems.map((it: any) => [it.id, it.qty, it.price]),
            contact: [formData.name, formData.phone, formData.email, formData.telegram],
            delivery: [
                isDigitalOnly, isIntl,
                formData.country, formData.city, formData.postal,
                formData.addressLine, formData.branch,
            ],
            payment: formData.paymentChoice,
            credits: [promoId, certCode, useBonus],
            pricing: [shipRegionChoice, locale, displayCurrency],
        });
        let h = 5381;
