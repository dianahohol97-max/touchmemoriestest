...
export default function AccountPage() {
    ...
    /**
     * Відкрити збережений макет у конструкторі.
     *
     * Уся передача — читання рядка, зменшені копії фото, підписані посилання,
     * відновлення розкладки і вибір режиму — живе в lib/editor/open-design.ts,
     * бо тим самим шляхом ходить місток /editor/open/[projectId], на який
     * ведуть картка замовлення в адмінці та листи-нагадування. Дві копії цього
     * коду розійшлися б за тиждень.
     */
    const reopenDesign = async (d: Design) => {
        const res = await openDesignInConstructor(d.id);
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: 16 }}>
                                        {designs.map(d => {
                                            ...
                                            const editUrl = isStarmap ? `/order/starmap`
                                                ...
                                                : d.source === 'editor' ? `/editor/open/${d.id}` : `/review/${d.id}`;
