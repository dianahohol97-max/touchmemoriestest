 ...
 * створити галерею»). The cabinet list now links here instead of expanding
 * panels inline: every step of preparing a gallery — upload, photo/cover
 * management, design, settings, sharing — lives on one screen.
 */
export default function GalleryEditorClient({ token, galleryId }: { token: string; galleryId: string }) {
  ...
  if (error || !gallery) return shell(<div style={{ color: '#991b1b', padding: '40px 0' }}>{error || 'Галерею не знайдено'}</div>);
