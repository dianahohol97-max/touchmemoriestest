...
export async function GET(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  // ВІДЕОПОБАЖАННЯ ЦЕЙ РОУТ НЕ ВІДДАЄ НІКОЛИ.
  //
  // Їх бачить лише пара, на своїй сторінці альбому. Ховати їх у клієнті було б
  // не приховуванням, а виглядом приховування: відповідь роута видно в будь-якому
  // браузері через панель розробника, разом із підписаними посиланнями на самі
  // ролики. Тому умова стоїть у самому запиті й обійти її з боку гостя нічим.
  //
  // І саме в запиті, до .limit(), а не відсівом після (гоча 13): побажань на
  // порядок менше за знімки, тож фільтр у JavaScript робив би сторінку
  // галереї то повною, то напівпорожньою залежно від того, скільки побажань
  // випадково трапилося серед 48 вибраних рядків.
