...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
    ...
    return NextResponse.json({
        ...
        // Only the first name — the page is opened from a link that may be
        // forwarded, and a surname adds nothing the customer needs.
        customerName: String(link.customerName || '').split(/\s+/)[0] || '',
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
    ...
    const path = `client-uploads/${link.orderNumber}/${Date.now()}_${Math.random().toString(36).slice(2, 8)}_${safeName}`;
