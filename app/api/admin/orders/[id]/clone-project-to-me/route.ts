    ...
    for (const p of projects) {
        const { data: copy, error } = await admin
            ...
            .insert({
                ...
                // Копія знає, для чого вона зроблена. З цієї позначки живуть
                // дві речі. Конструктор ховає «Зберегти та замовити», щоб
                // виправлення чужого макета не лягало в кошик дизайнера як
                // нова покупка. Список «Ваші виправлені макети» шукає чернетки
                // за нею, а не за збігом назви — перейменована чернетка більше
                // не зникає зі списку.
                fix_for_order_id: order.id,
            })
            .select('id')
            .single();
