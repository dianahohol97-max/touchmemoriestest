...
export async function POST(req: NextRequest) {
    try {
        ...
        let systemPrompt = "Ви експерт з контент-маркетингу для українського бізнесу з виготовлення фотокниг та фотопродуктів Touch Memories. Відповідай тільки українською мовою.";
        let userMessage = '';

        if (action === 'generate_structure') {
            userMessage = `Створіть детальну структуру статті для блогу на тему: ${payload.title}. 
            ...
            systemPrompt = "Ви SEO-спеціаліст для українського інтернет-магазину фотопродуктів Touch Memories. Відповідай тільки українською мовою. Повертай відповідь ТІЛЬКИ у форматі JSON, без жодного тексту до або після JSON.";
            ...
            systemPrompt = "Ви експертний автор блогу для українського бізнесу Touch Memories — виготовлення фотокниг, фотодруку та персоналізованих подарунків. Пиши природно, тепло, з експертністю. Відповідай тільки українською мовою.";
            userMessage = `Напишіть експертний вступний абзац для статті блогу з заголовком: ${payload.title}.
            Абзац має:
            - бути 3-4 речення
