...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
     ...
     *
     * Другою спробою був `headers` усередині `ImageResponse`, третьою — оця
     * відповідь, зібрана руками з потоку. Обидві дали те саме: `max-age`
     * доходить, `s-maxage` дорогою зникає, і край Vercel відповідає `MISS`
     * навіть на другий запит тієї самої адреси.
     *
     * ЩО МАЄМО НАСПРАВДІ, станом на 22.09.2026: `public, max-age=86400`. Це
     * означає, що браузер і клієнт, який поважає заголовок, тримають картинку
     * добу, а от край малює її щоразу заново. Стало краще, ніж було
     * (`max-age=0, must-revalidate`), але не так, як тут написано нижче.
     *
     * НЕ ПЕРЕПИСУЙ ЦЕЙ ЗАГОЛОВОК ІЩЕ РАЗ — три різні способи вже дали той
     * самий результат. Якщо край справді почне заважати, дешевше піти іншим
     * шляхом: малювати картинку один раз під час публікації (крон уже має
     * момент, коли стаття відкривається) і класти готовий PNG у сховище, а
     * звідси віддавати посилання на нього.
     */
