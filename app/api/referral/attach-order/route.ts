 ...
 *
 * ЗВЕДЕННЯ З ПОЛІТИКОЮ НА ВСТАВКУ (коміт 20fdb32b, паралельна сесія).
 *
 * Та сесія знайшла в тій самій точці іншу ваду: політика «Customers create
 * orders» на public.orders дозволяє INSERT будь-кому з `with check (true)`,
 * тобто підроблений запит міг би заявити payment_status='paid'. Її міграція
 * лежить незастосованою в docs/proposed-migrations і чекає на рішення Діани.
 *
 * Конфлікту з цим роутом немає, і ось чому конкретно. Політика гейтить ЛИШЕ
 * INSERT і лише під ключем anon — саме так браузер створює рядок. Роут працює
 * під службовим ключем (getAdminClient), який RLS обходить цілком, і робить
 * UPDATE, якого політика не стосується взагалі. Порядок при цьому такий:
 * вставка з браузера під політикою → цей роут під службовим ключем → рахунок
 * Монобанку, який читає суму з уже виправленого рядка. Жоден крок не блокує
 * наступний.
 *
 * Дві умови політики навіть збігаються з тим, що робить роут: вона пришпилює
 * payment_status='pending' на вставці, а роут відмовляється чіпати оплачене
 * замовлення. Тож коли Діана її застосує, поведінка не зміниться ні на крок.
 *
 * ЦЕЙ РОУТ — МІСТОК, А НЕ КІНЦЕВИЙ ДИЗАЙН. Той самий коміт правильно пише, що
 * повне рішення — перенести обидві браузерні вставки на сервер; тоді submit
 * визначатиме партнера сам, публічна політика на вставку стане непотрібною, а
 * цей роут можна буде видалити. Поки вставки клієнтські, він закриває те, що
 * втрачається щодня.
 */
