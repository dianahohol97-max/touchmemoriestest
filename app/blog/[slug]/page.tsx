        ...
        <div style={{ backgroundColor: 'white', minHeight: '100vh', fontFamily: 'var(--font-primary)' }}>
            ...
            <main style={{ paddingTop: '100px', paddingBottom: '100px' }}>
                <article style={{ maxWidth: '800px', margin: '0 auto', padding: '0 24px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #f1f5f9', borderBottom: '1px solid #f1f5f9', padding: '20px 0' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                {post.author_avatar ? (
                                    ...
                                    <img src={post.author_avatar} alt={post.author_name} style={{ width: '48px', height: '48px', borderRadius: '50%' }} />
                                ) : (
                    ...
                    {/* Cover Image */}
                    {post.cover_image && (
                        <div style={{ width: '100%', height: '500px', position: 'relative', borderRadius: '24px', overflow: 'hidden', marginBottom: '48px', backgroundColor: '#f8fafc' }}>
                    ...
                    {post.tags && post.tags.length > 0 && (
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '48px' }}>
                            {post.tags.map((tag: string) => (
                                <Link key={tag} href={`/blog/tag/${tag}`} style={{ backgroundColor: '#f1f5f9', color: '#475569', padding: '6px 16px', borderRadius: '100px', fontSize: '14px', fontWeight: 600, textDecoration: 'none', transition: 'background 0.2s', ':hover': { backgroundColor: '#e2e8f0' } } as any}>
                                    #{tag}
                                </Link>
                            ))}
                        </div>
                        ...
                        <div style={{ borderTop: '2px dashed #f1f5f9', paddingTop: '40px', marginBottom: '60px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px' }}>
                                {relatedProducts.map((p: any) => (
                                    <Link key={p.id} href={`/products/${p.slug}`} style={{ border: '1px solid #f1f5f9', borderRadius: '16px', padding: '16px', textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', transition: 'border-color 0.2s', ':hover': { borderColor: '#cbd5e1' } } as any}>
                    ...
                    <div style={{ backgroundColor: '#1e293b', borderRadius: '24px', padding: '40px', color: 'white', textAlign: 'center', marginBottom: '80px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <Link href="/constructor" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '16px 32px', backgroundColor: '#3b82f6', color: 'white', borderRadius: '16px', fontWeight: 800, fontSize: '16px', textDecoration: 'none', transition: 'transform 0.2s', ':hover': { transform: 'scale(1.05)' } } as any}>
...
const shareBtnStyle = { display: 'flex', alignItems: 'center', justifyContent: 'center', width: '40px', height: '40px', borderRadius: '50%', backgroundColor: '#f1f5f9', color: '#64748b', border: 'none', cursor: 'pointer', transition: 'all 0.2s' };
