...
-- Другий прохід того ж дня: партнер може зареєструватися сам — через форму на
-- сайті або через власний кабінет фотографа, — а менеджер до того місяць вів
-- його в директі. Такі випадки не створюють партнера (він уже є), а лише
-- записують менеджера, тож заявки бувають двох видів:
--   'create' — оформити ліда партнером, потрібне підтвердження адміністратора
--   'claim'  — зарахувати менеджеру вже існуючого партнера без менеджера
-- claim не блокується підтвердженням (рішення Діани, 2026-08-06: менеджер має
-- могти підписати партнера на себе сам), але лишає рядок в історії — саме
-- заради видимості, хто, кого і коли собі зарахував.
ALTER TABLE partner_activation_requests
