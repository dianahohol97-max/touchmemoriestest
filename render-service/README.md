...
## Коли Railway має перезбиратися

`railway.json` у цій теці тримає `build.watchPatterns`, і сенс у нього один:
сервіс перезбирається тільки тоді, коли змінився код самого сервісу, тобто
щось усередині `render-service/`. Доти кожен пуш у `main` — виправлення блогу,
правило доступу, опція в каталозі — підміняв контейнер під рендером, що триває.
Браузер помирав посеред прогону з «Target page, context or browser has been
closed», а наступний виклик діставав від Railway 502 «Application failed to
respond». За дві доби це дало вісімнадцять записів у журналі помилок і
залишило переекспорт TM-001352 узагалі без файлів.

Якщо Railway колись перестане підхоплювати цей файл, те саме є в дашборді:
**Service → Settings → Build → Watch Paths**, значення `/render-service/**`.
Значення з дашборда і значення з файлу описують одне й те саме, тож достатньо
одного з них.

Перевірка, що правило працює, займає два пуші. Пуш, який чіпає
`render-service/`, має дати новий деплой — відкрий `{railway-url}/health` і
звір `commit` із верхівкою `main`. Пуш, який `render-service/` НЕ чіпає, нового
деплою давати не має: у списку деплоїв Railway після нього нічого не
з'являється, а `commit` у `/health` лишається старим, і це тепер нормально.
