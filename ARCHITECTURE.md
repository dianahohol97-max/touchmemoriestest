...
4. **Webhook handler** (`app/api/monobank/webhook/route.ts`) does, in order:
   ...
   - **Everything that must happen once per order hangs on a single `isFirstPaidTransition` flag** (added 2026-09-13), computed once right before the UPDATE and used by all eight side effects: stock deduction, the payment-received email, the work-chat ping, production automation, fiscalisation, print render, designer handoff and certificate redemption. Until then the same expression `existingOrder.payment_status !== 'paid'` was written out eight times, and that is exactly how the duplicate-email bug survived: `existingOrder` is read ONCE before the UPDATE and never re-read, so every copy of the test returns the same answer inside one request, and TWO separate blocks each sent the «payment received» letter believing itself to be the only one. Customers got two identical letters about two seconds apart on 70 of 71 paid orders — the gap was simply how long the first (awaited) call took. Do NOT "fix" a repeat of this by re-reading `payment_status` after the UPDATE: immediately after it the status IS `paid`, so all eight conditions would turn false at once and silently disable stock deduction, fiscalisation, the designer handoff and certificate redemption along with the mail. The condition belongs on the UPDATE's outcome, not on re-read state; the race itself is already closed by the RPC, which returns the order id only to the caller that won it while everyone else leaves via `idempotent: true`.
...
- **`email_logs`** — the outgoing-mail log for everything the shop sends a customer, surfaced as the «Листування з клієнтом» card on the order page. Brevo is the only transport (`lib/email/resend.ts` keeps the old Resend export names but sends through Brevo). **Write to it only through `logOutgoingEmail()` in `lib/email/log-outgoing.ts`** — never with a hand-rolled insert. That function has a SIDE EFFECT beyond logging, and its name is narrower than what it does: it also sets `orders.manager_id` when the order has none, so a manager is assigned from any of the five send paths at once. Deliberate trade-off, documented in the function's own header; a failed send assigns too, because the person took the order on either way. The insert used to be written out in four places and they drifted: two recorded `body`, none recorded `provider_message_id` (0 of 255 rows), and the «замовлення прийнято» mail from `create-invoice` went through a `.catch(() => {})` that made a provider refusal invisible in both the log and the UI. The helper always writes a row — on success AND on failure — and `readSendOutcome()` normalises the two different result shapes the transports return (`sendEmail` gives `{success,data,error}`, `getResendClient().emails.send()` gives `{data,error}`), which is where the message id kept getting lost. Columns: `order_id`, `customer_email`, `template`, `subject`, `body` (plain-text snapshot — template mails are stripped to text by `htmlToTextSnapshot()` because the card renders it as text), `provider_message_id`, `status`, `error`, `sent_at`, plus `sent_by` → `staff(id)` and `sent_by_name` (added 2026-09-13). `sent_by_name` duplicates the staff name ON PURPOSE, so the log stays readable after a staff row is deleted; both being NULL means the send was automatic (webhook, cron, customer-side checkout). **Delivery (added 2026-09-13):** `delivery_status` / `delivery_detail` / `delivered_at` / `delivery_updated_at` carry what Brevo reports AFTER hand-off, fed by `POST /api/webhooks/brevo`; `failure_kind` (`quota`|`provider`|`config`|`precheck`) says why a hand-off failed, so an exhausted daily cap reads as its own cause rather than a generic error. `delivery_status` is deliberately NOT more values on `status`: `status` answers «did we hand it over» (sent/failed) and `delivery_status` answers «what happened next», and merging them would force a mail we handed over successfully but Brevo could not deliver to read as our own failure. Event freshness is decided by the `ts` INSIDE the event, never by arrival time — Brevo does not guarantee order and a deferred can land after a delivered — and the rule lives in `lib/email/delivery-events.ts` (pure, tested in `tests/email-delivery.test.ts`) alongside the event map, the message-id normaliser (both sides are stripped of angle brackets and lowercased, or matching fails silently) and `describeMailRow()` which the order card renders. The webhook needs `BREVO_WEBHOOK_SECRET` in the `x-webhook-secret` header — Brevo does not sign webhooks, unlike Monobank — answers 503 without it, replies 202 BEFORE doing the work via `after()` from next/server (a floating promise does not survive on Vercel), and treats an unknown event type as a logged 2xx rather than an error. Campaign mail is NOT covered: `email_campaign_queue` stores no provider message id and the drain writes no `email_logs` row, so delivery statuses today cover per-order mail only. The acting person is resolved by `resolveActingStaff()` in `lib/auth/guards.ts` — `staff` has no auth-user column, it is matched by email — and is passed in the request body to `/api/email/transactional`, which is called server-to-server with the cron secret and so never sees the manager's cookies.
- **Two different shop addresses, and only one of them is a mailbox.** Mail is SENT from `hello@touchmemories.com.ua` (`BREVO_FROM_EMAIL`) — that domain is verified in Brevo so sending works, but **there is no mailbox behind that address at all**, and anything written to it disappears. The address customers may actually be given is `SHOP_CONTACT_EMAIL` in `lib/email/contact-address.ts` (`touch.memories3@gmail.com`), which is live, sits on the public contacts page and receives every site form; it is also the `Reply-To` default in `lib/email/brevo.ts` (tested in `tests/email-reply-to.test.ts`). The constant exists because the address had already spread through the code in four different spellings, one of them `hello@touchmemories.ua` without the `.com`. The constant was introduced 2026-09-08 but the email TEMPLATES were not moved onto it until 2026-09-13; six of them — `AbandonedCartEmail`, `PaymentReminderEmail`, `OrderCancelledEmail`, `WelcomeSeriesEmail`, `WinBackEmail`, `ReviewRequestEmail` — still invited the customer to write to the dead address. In two of those (`AbandonedCartEmail`, `WinBackEmail`) the invitation was worded as the unsubscribe instruction («Якщо більше не хочете отримувати такі листи — напишіть нам на …»). That did NOT leave anyone trapped: marketing mail already carries a working «Відписатися від розсилки» link plus `List-Unsubscribe` / one-click headers, both added by `withUnsubscribeFooter()` in `lib/email/unsubscribe.ts` whenever the caller passes `unsubscribe` (the abandoned-cart, win-back and welcome-series crons all do). What it did leave is a second, competing route out that was broken — a reader who followed the body text instead of the footer wrote to a mailbox that does not exist, got no answer, and stayed subscribed. The body sentence is redundant with the footer and should eventually be deleted rather than merely re-pointed. **Rule: never hardcode a contact address in a template — import the constant.** `hello@…` remains correct only as the SENDER, where a missing mailbox does not matter.
- **Resend guard on `/api/email/transactional`** (added 2026-09-13) — the same `action` + `orderId` is not sent twice inside `RESEND_GUARD_WINDOW_MINUTES` (60). The rule lives in `lib/email/resend-guard.ts` (pure, tested in `tests/email-resend-guard.test.ts`), NOT in the callers: the `paid` action alone has three senders left (the Monobank webhook, «Позначити оплаченим», «Перевірити оплату»), and patching each one leaves the fourth open to whoever adds it next. The guard runs before rendering, so a suppressed letter never builds its HTML, and it answers **200 with `deduplicated: true`** rather than an error — the caller asked for «the customer must know», the customer already knows, and an error here would make `send-payment-link` and `create-invoice` write a failure row that shows the manager red where nothing is wrong. A suppressed send writes NO `email_logs` row, because nothing was sent. **`placed` is deliberately exempt**: that is the payment-link letter and the «Надіслати посилання клієнту» button exists precisely to send it again — in the log those pairs sit hours apart (5 of 96, all to unpaid orders), unlike the two-second `paid` duplicates. An unknown or non-string action is guarded rather than let through, because a silent duplicate costs more than a silent pause. The window is a constant, not «ever», because an order lives for weeks: a shipping letter after a re-issued TTN is an event, not a duplicate.
