...
- **What leaves the inbox:** dialog text does not stay in `admin/social-inbox`. The watchdog cron quotes the client's last message into the work chat; `/unanswered` does the same on demand; and `lib/chatbot/client-chat-lookup.ts` feeds up to 40 messages of a matched dialog to the Anthropic API whenever somebody asks Софія about an order. Access to the inbox itself is NOT the RLS policy (`is_admin_user()`, 4 emails) — the route reads with the service key behind `requireSection('ai','view')`, so every one of the 14 active staff can read any dialog.
