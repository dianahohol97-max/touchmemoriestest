...
- **Proactive team alerts** (all to the `/alerts_here` destination, falling back to Diana's chat with the bot): 💰 paid-order ping fired from the Monobank webhook on the first transition to paid (test orders excluded via `lib/automation/test-orders.ts`); ☀️ morning digest `/api/cron/work-chat-digest` (06:00 UTC) = the `/status` summary plus deadlines due within 2 days; 🔔 unanswered-dialogs watchdog (see above).
