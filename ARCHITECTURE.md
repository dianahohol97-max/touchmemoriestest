...
- **Work-chat order mentions** (`lib/chatbot/work-chat-monitor.ts`): non-command group messages mentioning an order (TM-…/CRM-…/bare 5-digit KeyCRM id; numbers followed by грн/₴ are prices and skipped) are captured silently — an `order_history` note (action `work_chat_note`) + deadline TIGHTENED to any dd.MM date in the message (earlier-only). The bot acknowledges with a ✍ reaction, never a chat message. Needs the bot as group admin to see non-command messages.
