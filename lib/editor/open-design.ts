...
export async function openDesignInConstructor(projectId: string): Promise<OpenDesignResult> {
     ...
     * шість несуть тонку позначку `{ id }` без опцій — за них тепер відповідає
     * сервер, дивіться наступний абзац.
     *
     ...
     * кошика, далі позиція замовлення з сервера, далі `enableEndpaper`, далі
     * замкнено. Жодне слабше джерело не може зняти оплату, яку назвало
     * сильніше.
     */
    let endpaperPaid = resolveEndpaperPaid(config.endpaperPaid, cp.options);
    /**
     * МАКЕТИ, У ЯКИХ `cart_payload` — САМА ЛИШЕ ПОЗНАЧКА `{ id }`.
     *
     * Таких 23.09.2026 було шість із двадцяти на замовленнях з оплаченим
     * форзацом. Два з них — TM-001309 і TM-001091 — довелося розблоковувати
     * руками, дописуючи `endpaperPaid` прямо в конфіг макета, бо в браузері
     * джерел для них не лишалося зовсім. Справжня відповідь весь час лежала в
     * `orders.items`, куди клієнтові не дає зазирнути RLS, тож питаємо про неї
     * сервер.
     *
     * Питаємо ТІЛЬКИ тоді, коли два перші джерела промовчали: там, де оплата
     * вже названа, зайвий виклик нічого не додасть, а відкриття макета має
     * бути швидким. Помилка мережі лишає все точно таким, яким воно було до
     * цього рядка, тобто відкатом на `enableEndpaper`.
     */
    if (!endpaperPaid) {
        try {
            const res = await fetch(`/api/projects/${row.id}/endpaper-paid`);
            if (res.ok) {
                const info = await res.json().catch(() => null);
                if (info?.paid) endpaperPaid = { first: !!info.paid.first, last: !!info.paid.last };
            }
        } catch {
            // Мовчки: далі працює той самий відкат, що працював досі.
        }
    }
