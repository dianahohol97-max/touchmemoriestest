...
if (layout === 'sp-1-top-strip') return [S(0, 0, 0, W, H*0.4)];
  ...
  if (layout === 'p-7-diagonal')     return Array.from({length:7},(_,ii)=>S(ii,(ii%3)*(W/3+g),(Math.floor(ii/3))*(H/3),W/3-g,H/3-g));
  ...
  if (layout === 'p-8-diagonal')     { const sz=W*0.35; return Array.from({length:8},(_,ii)=>S(ii,(ii%2)*((W-sz-g)/1+g+sz/2-(ii%2)*(sz/2+g)),(Math.floor(ii/2))*(H/4),sz,H/4-g/2)); }
  ...
  if (layout === 'p-9-diagonal')     { const sz=H*0.33; return Array.from({length:9},(_,ii)=>S(ii,(ii%3)*(W/3),Math.floor(ii/3)*(sz),W/3-g,sz-g)); }
