 ...
 * І ГЛЯНЦЕВИХ ЖУРНАЛІВ і 3% з решти. Ті самі числа, що й у ручному оформленні
 * в адмінці та в автоматичному зарахуванні фотографів, зібрані в одному місці,
 * щоб заявки менеджерів не завели четвертий набір ставок.
 *
 * Журнали в назві колонки не видно, і саме через це тексти для людей півтора
 * місяця обіцяли за журнал три відсотки замість пʼяти: `travelbook_rate` читали
 * буквально. Що саме потрапляє в пʼятивідсотковий кошик, вирішує одна функція —
 * `isPremiumRateItem` у lib/agency/commission.ts, — і звірятися треба з нею, а
 * не з іменем поля.
 */
