...
export async function verifyOrderPrint(orderId: string): Promise<OrderPrintVerdict> {
    ...
    for (const [i, p] of (projects || []).entries()) {
        ...
        //
        // Три родини стояли в підбірці конструктора з прапорцем кирилиці, не
        // маючи жодного кириличного гліфа: Lato, Poppins і Schibsted Grotesk.
        // Підбірка їх більше не пропонує, але дванадцять збережених макетів
        // несуть Lato і чотири Poppins, і український текст у них друкується
        // системним шрифтом. Сюди ж потрапляє родина, якої в нашому наборі
        // немає взагалі, як Georgia: вона підміняється цілком, а не частиною.
        //
