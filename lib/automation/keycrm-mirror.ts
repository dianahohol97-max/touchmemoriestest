...
function toOrderRow(crm: KeycrmOrder, existing?: { id: string; deadline?: string | null; custom_attributes?: any }) {
    ...
    const items = crm.products.map(p => ({
        ...
        // Собівартість рядка з CRM. Єдине, що тут додано 16.09.2026: до цього
        // дзеркало читало товарний рядок повністю і `purchased_price`
        // викидало, тож єдине місце в системі, де собівартість лежить по
        // варіантах, до нас не доїжджало. Без неї валова маржа рахувалася з
        // `products.cost_price` за точною назвою, а назви з CRM у довіднику
        // сайту здебільшого немає: за вересень собівартість знаходилася для
        // 86 позицій із 572.
        ...(p.cost !== null ? { cost_price: money(p.cost) } : {}),
