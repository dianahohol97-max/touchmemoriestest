...
21. **Формат фотографії визначають байти, а не розширення** (Diana, 2026-09-22). Правило: **у сховище їде або формат, який читає будь-який браузер, або байти, які ми перемалювали самі.** Історія — TM-001343 і TM-001244, обидва тревелбуки однієї клієнтки. Макет у базі був бездоганний: чотирнадцять сторінок, сорок слотів, у кожному фото, усі сорок вісім файлів у сховищі. У друк поїхало одинадцять порожніх аркушів. Домалювалися рівно три знімки, і це були єдині три файли проєкту з довгою стороною понад 4500 px — решта сорок два були HEIC під іменем `.jpg`. `isHeic()` дивився тільки на MIME і на розширення, тож не впізнавав їх, а три місця в редакторі (`BookLayoutEditor.tsx`, рядки 3914, 4437, 5485) вантажать оригінал байт у байт із прибитим `contentType: 'image/jpeg'`. Safari на її техніці HEIC декодує рідно, тому і вона, і ми в адмінці бачили справний макет, а headless Chromium на Railway малював порожній слот. Тепер формат читається з перших байтів (`lib/image-signature.ts`, чистий і під тестами), HEIC конвертується навіть із чужим іменем, нерозпізнане растрове перемальовується через canvas ТОГО САМОГО браузера, який щойно показав фото клієнтці, а справжній JPEG лишається недоторканим — перекодувати камерний JPEG означає повернути «розмитість», яку вже одного разу прибирали. Друга половина поломки була в тиші: рендер-сервіс рахує `imgReport.broken`, але писав про це `console.warn` у логи Railway і вивантажував порожній аркуш як успішний експорт, тож `order_files` показував «макет готовий». Тепер це відмова сторінки. **Дірка в макеті видна, а порожній аркуш із написом «готово» — ні.** Перевірка, яка ловить саме такі аркуші, у нас була весь час (`lib/print/verify-order-print.ts`), і запускає її тільки кнопка в адмінці — тому про обидва замовлення ми дізналися очима.
