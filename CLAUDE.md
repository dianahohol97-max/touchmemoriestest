...
14. **Запит до великої таблиці пишеться з пагінацією ОДРАЗУ** (Diana, 2026-09-14). Правило: **будь-яке читання з `orders`, `customers`, `social_messages`, `social_conversations`, `email_logs` і `projects` має або цикл із `.range()`, або свідомий `.limit()`, або фільтр по ключу одного батьківського запису. Дивитися на сьогоднішню кількість рядків НЕ треба — саме це нас і підводило.** PostgREST віддає щонайбільше тисячу рядків і не каже про це нічим: відповідь просто коротша за правду, помилки немає, у журнал не потрапляє ніщо. За один день 14.09.2026 пастка спрацювала чотири рази — список клієнтів (1000 із 1280), дашборд (1000 із 1107, через що «потребує дизайнера» показував 83 замість 122), аналітика на періоді понад сорок сім днів (квартал недоливав 138 218 ₴) і один діалог у скриньці (1000 із 3 021). Щоразу помічали випадково. Шість таблиць у списку — ті, що ростуть від роботи магазину, а не від рішення адміністратора: довідник на кшталт `products` росте тоді, коли Діана щось додає, і тисячу рядків там видно заздалегідь, а ці наповнюються самі і перетинають межу без жодної зміни в коді. Перевірка: `node scripts/unpaginated-queries.mjs` — він друкує ті, що треба переписати, окремо від тих, що обмежені батьківським ключем, і повертає код 1, якщо перший список не порожній. Поточний розбір із числами — `docs/unpaginated-queries.md`. Сусідня гоча 13 про інший бік тієї самої монети: там ліміт є, але фільтр стоїть після нього.
   ...
   **Фільтр по батьківському ключу не рятує, коли в одного батька понад тисячу рядків** (Diana, 2026-09-24). До списку додано `wedding_photos` і `photographer_gallery_photos`: їх наповнюють гості й фотографи, а не адміністратор, і одна галерея сама дає до двох тисяч рядків. Крон `cleanup-galleries` читав фото галереї без `.range()`, тобто щонайбільше тисячу, стирав зі сховища лише їх, а тоді `delete().eq('gallery_id', …)` прибирав з бази всі рядки й ставив `files_purged_at`. Файли після тисячного лишалися в R2 назавжди без рядка, і знайти їх уже нічим. Скрипт цього не бачив би навіть із таблицею в списку, бо `.eq('gallery_id', …)` він зараховує до «обмежених батьківським ключем». Усі читання цієї таблиці тепер ідуть через `lib/photographers/gallery-photos.ts` (сторінки по тисячі, сортування `created_at`, потім `id`). Порядок очищення розписано в `lib/photographers/gallery-cleanup.ts`: рядок видаляється тільки після того, як сховище підтвердило, що файлу немає, і тільки за своїм `id`. `removeFiles()` повертає `null` лише тоді, коли зникли всі файли: вимкнений R2 і непорожнє `Errors` у відповіді `DeleteObjects` тепер вважаються помилкою.
...
19. **Збережений макет не показують з оригіналів** (Diana, 2026-09-20). Правило: **на екран іде зменшена копія, оригінал лишається тільки для друку.** TM-001342 має 68 знімків на 184 МБ, і повторне відкриття тягнуло їх усі: клієнтка порахувала за нас — «13 минут прошло, а загрузилось только 14 из 67», і зайшла вона заради одного напису на обкладинці. Вага тут навіть не головне: кожен оригінал це близько дванадцяти мегапікселів, тобто майже п'ятдесят мегабайтів растру, який браузер розкодовує заради шістдесятипіксельної мініатюри. Поруч із оригіналом тепер лежать копії на 1600 px для полотна і на 360 px для стрічки (`lib/editor/photo-variants.ts`), їх робить конструктор під час збереження, а старі макети доганяє сервер через sharp (`POST /api/projects/[id]/photo-variants`). **Оригінал у `path` не заміняється ніде**: клієнтський знімок екрана вимкнено (`SKIP_CLIENT_HTML2CANVAS`), макет для друку збирає Railway саме з `path`, і `/api/print/[projectId]` підписує його. Додаючи ще один розмір, памʼятай, що шляхи копій рахуються в одному місці — `lib/editor/photo-variant-paths.ts`, — бо розбіжність між браузером і сервером означає копію, якої ніхто не знайде. Окремо: `reopenDesign` мусить передавати фото поле `path`, інакше конструктор вважає відновлений знімок новим і на першому ж збереженні заливає всі оригінали вдруге.
   ...
   **Те саме правило діє для клієнтських галерей фотографів** (Diana, 2026-09-24). До цього сітка, лайтбокс, обкладинка і мініатюри в кабінеті показували оригінали, і повна прокрутка галереї Ірини Владової тягнула всі 3,66 ГБ. Тепер поруч з оригіналом лежать JPEG-копії на 640, 1280 і 2048 px, а браузер обирає потрібну через `srcset`. Шляхи рахуються тільки в `lib/photographers/gallery-variant-paths.ts`. Копії ріже сервер через sharp, а не браузер фотографа, бо той під час аплоаду вже зайнятий чотирма паралельними завантаженнями. Оригінал лишається в `url` для завантаження поштучно й архівом, і в квоту йде тільки він. Кожне видалення фото мусить іти через `galleryFilesToRemove`, інакше копії стануть сиротами в R2.
