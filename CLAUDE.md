...
**Те саме перед КОЖНОЮ перевіркою, а не лише на старті** (Diana, 2026-09-17). Паралельна сесія пише в той самий `main` протягом дня, і тричі поспіль ми витрачали час на з'ясування того, що насправді змінив хтось інший: прибраний рядок крона виглядав як незареєстрований крон, а видалені як мертві маршрути — як ті, що треба лагодити. Перед тим, як щось перевіряти чи пояснювати, спершу `git fetch origin main && git log --oneline HEAD..origin/main`.
...
18. **Макет належить рядку кошика, і це треба перевіряти** (Diana, 2026-09-20). Правило: **щоразу, коли макет зберігається під ключем позиції, звіряй, що це саме її макет.** Історія — TM-001342. Клієнтка зібрала дві РІЗНІ тревелбуки в одному конструкторі, жодного спільного знімка між ними, і оплатила обидві одним замовленням на 2052 ₴. Рядок, оцінений за 65 фото і 16 сторінок, отримав під свій ключ 68 фото і 22 сторінки другої книги, а перша книга лишилася непривʼязаною чернеткою. У друк поїхали б дві копії однієї книги, і другої клієнтка не отримала б узагалі. Помітила це менеджерка очима, звіряючи картку з CRM. Ключ був увесь час — `design_{cartItemId}` у сховищі вкладки і `projects.cart_payload->>id` у базі — не було перевірки, що під ключем лежить той самий макет, тож пізніший стан редактора мовчки заміщав ранішній. Лікування в `lib/orders/design-ownership.ts`: макет несе позначку `cartItemId`, і оформлення та `save-design` відмовляються класти його під чужий ключ. **Відмова тут краща за запис**: позиція без макета видна сторожу і перевірці файлів, а позиція з чужим макетом виглядає справною рівно до друку. Друга половина тієї самої поломки: ключ рядка не зберігався в самому замовленні — 579 замовлень за тридцять днів і в жодному його немає, бо оформлення губило `it.id` у тому самому місці, де колись губило `personalization_note`. Тепер зберігається, і на ньому стоїть п'ята ознака сторожа тихих втрат. Звіряти **по ключу, а не по лічильнику**: прохід по живій базі за тридцять днів дав чотири замовлення, де макетів менше за книги, і жодне не було поломкою — два з дизайнером, одне з готовими файлами, одне з поясненням людини в примітці.

19. **Збережений макет не показують з оригіналів** (Diana, 2026-09-20). Правило: **на екран іде зменшена копія, оригінал лишається тільки для друку.** TM-001342 має 68 знімків на 184 МБ, і повторне відкриття тягнуло їх усі: клієнтка порахувала за нас — «13 минут прошло, а загрузилось только 14 из 67», і зайшла вона заради одного напису на обкладинці. Вага тут навіть не головне: кожен оригінал це близько дванадцяти мегапікселів, тобто майже п'ятдесят мегабайтів растру, який браузер розкодовує заради шістдесятипіксельної мініатюри. Поруч із оригіналом тепер лежать копії на 1600 px для полотна і на 360 px для стрічки (`lib/editor/photo-variants.ts`), їх робить конструктор під час збереження, а старі макети доганяє сервер через sharp (`POST /api/projects/[id]/photo-variants`). **Оригінал у `path` не заміняється ніде**: клієнтський знімок екрана вимкнено (`SKIP_CLIENT_HTML2CANVAS`), макет для друку збирає Railway саме з `path`, і `/api/print/[projectId]` підписує його. Додаючи ще один розмір, памʼятай, що шляхи копій рахуються в одному місці — `lib/editor/photo-variant-paths.ts`, — бо розбіжність між браузером і сервером означає копію, якої ніхто не знайде. Окремо: `reopenDesign` мусить передавати фото поле `path`, інакше конструктор вважає відновлений знімок новим і на першому ж збереженні заливає всі оригінали вдруге.

20. **Кожен лист людині лишає слід у журналі** (Diana, 2026-09-17). Правило: **новий відправник пишеться через `sendLoggedEmail` із `lib/email/send-logged.ts`, а не через `sendBrevoEmail` напряму.** До 16.09.2026 в `email_logs` були тільки листи навколо замовлення — 298 рядків на три шаблони, — хоча листи шлють 44 файли трьома різними шляхами: прямим `sendBrevoEmail`, обгорткою `lib/email/resend` і сирим `fetch` на `api.brevo.com`. Останній був найгірший: він обходив `reserveEmailQuota`, тобто їв денний ліміт тарифу, не зменшуючи наш власний бюджет, і в день розсилки транзакційний лист про оплату міг не піти. Сирих викликів більше немає, пошук `api.brevo.com` поза `lib/email/brevo.ts` має лишатися порожнім. Журнал пишеться на КОЖНУ спробу, і на успіх, і на відмову: «не дійшло» і «не відправляли» мусять виглядати по-різному. `email_automation_log` — це не журнал, а захист від повторів, і рядок у ньому ставиться тільки після успіху, інакше відмова Brevo назавжди закриє людині цей крок автоматизації. Листи НАМ САМИМ на `touch.memories3` не заводимо свідомо, масову розсилку ведемо в її власній черзі `email_campaign_queue`. `provider_message_id` є з 13.09.2026, тож 256 старіших рядків статусу доставки не отримають ніколи. Повний перелік відправників, обсяги за тиждень і мертві шляхи — `docs/email-senders.md`; там же три автоматизації, які виглядають живими, а не шлють нічого: привітання з днем народження, сповіщення про зміну статусу і прохання про відгук.

21. **Формат файлу визначають байти, а не розширення** (Diana, 2026-09-22). Правило: **перш ніж вирішувати, що робити з фотографією, прочитай її перші байти.** Замовлення TM-001343 (травелбук, проєкт `a64540ba`) поїхало в друк одинадцятьма порожніми аркушами з чотирнадцяти, і макет у базі був цілий: чотирнадцять сторінок, сорок слотів, у кожному стоїть `photoId`, усі сорок вісім файлів лежать у сховищі. Домалювалися рівно три знімки, і це єдині три файли проєкту з довгою стороною понад 4500 px. У TM-001244 тієї самої клієнтки трьома тижнями раніше дванадцять порожніх аркушів із п'ятнадцяти і вижили знову рівно два знімки 4284×5712 — п'ять фото, два незалежні замовлення, збіг стовідсотковий. Решта її файлів — HEIC під іменем `.jpg`: `isHeic()` дивилася тільки на MIME і розширення, тож не впізнавала їх і не конвертувала, а редактор вантажив оригінал байт у байт із прибитим `contentType: 'image/jpeg'` — це три місця в `components/BookLayoutEditor.tsx`, чернетка, завантаження при замовленні і знімок макета. Safari на техніці клієнтки HEIC декодує рідно, тому і вона, і ми в адмінці бачили справний макет; headless Chromium на Railway, яким рендериться друк, такого не читає і ставив порожній слот. Рішення тепер приймає `lib/image-signature.ts` за першими 32 байтами, і в ньому важливий порядок: **AVIF перевіряється ПЕРШИМ**, бо марки `mif1` і `msf1` носить і він теж, а AVIF браузери читають і перекодовувати його не можна. Камерний JPEG віддається байт у байт із тієї самої причини, з якої колись прибирали «розмитість»: подвійне стиснення видно. Лічба для тоста «Конвертую N фото з iPhone…» іде тією самою умовою (`needsHeicConversion`), інакше набір таких файлів конвертується хвилину-дві в цілковитій тиші. Друга половина поломки — рендер це БАЧИВ: `imgReport.broken` рахувався і йшов у `console.warn` у логи Railway, а аркуш вивантажувався як успішний експорт, тож `order_files` показував «макет готовий»; тепер розворот кидає помилку і потрапляє в `failedSpreads`, бо дірка в макеті видна, а порожній аркуш із написом «готово» не видно ніколи. Окремо про перевірку: `lib/print/verify-order-print.ts` таку поломку ловить — порожній аркуш там ознака номер два, — але запускає її тільки кнопка в адмінці, тож про обидва замовлення ми дізналися очима, а не від неї.
