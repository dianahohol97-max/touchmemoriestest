                    ...
                    <div style={{ width: pageW, height: cH, flexShrink: 0, position: 'relative', background: backBg, borderRight: '2px solid rgba(0,0,0,0.12)' }}
                      ...
                      {backPhoto && (
                        <div style={{ width:'100%', height:'100%', position:'absolute', inset:0, overflow:'hidden', cursor:'grab' }}
                          ...
                          <img src={backPhoto.preview} style={{ width:`${Math.max(100,bZoom*100)}%`, height:`${Math.max(100,bZoom*100)}%`, objectFit:'cover', position:'absolute', top:'50%', left:'50%', transform:`translate(-50%,-50%) translate(${Math.max(-Math.max(15,((bZoom-1)/bZoom)*50), Math.min(Math.max(15,((bZoom-1)/bZoom)*50), (bCropX-50)*-0.5))}%, ${Math.max(-Math.max(15,((bZoom-1)/bZoom)*50), Math.min(Math.max(15,((bZoom-1)/bZoom)*50), (bCropY-50)*-0.5))}%)`, userSelect:'none', pointerEvents:'none' }} draggable={false}/>
                        </div>
                      )}
                      {backPhoto && isPrinted && (
                        <>
                            ...
                            <>
                              ...
                              <div style={{ width:'100%', height:'100%', overflow:'hidden', position:'relative', cursor: photoEditSlot === key ? 'crosshair' : 'default', padding: pageGap > 0 ? pageGap : 0, boxSizing:'border-box' }}
                                ...
                                <img src={photo.preview} draggable={photoEditSlot !== key}
                                  ...
                                  style={{ width:`${Math.max(100,(slot!.zoom||1)*100)}%`, height:`${Math.max(100,(slot!.zoom||1)*100)}%`, objectFit:'cover', position:'absolute', top:'50%', left:'50%', transform:`translate(-50%,-50%) translate(${Math.max(-Math.max(15,(((slot!.zoom||1)-1)/(slot!.zoom||1))*50), Math.min(((slot!.zoom||1)-1)/(slot!.zoom||1)*50, ((slot!.cropX??50)-50)*-0.5))}%, ${Math.max(-Math.max(15,(((slot!.zoom||1)-1)/(slot!.zoom||1))*50), Math.min(((slot!.zoom||1)-1)/(slot!.zoom||1)*50, ((slot!.cropY??50)-50)*-0.5))}%)`, userSelect:'none', cursor:photoEditSlot===key?'grab':'default', display:'block', touchAction: photoEditSlot===key ? 'none' : 'auto' }}/>
                                {/* Zoom hint + badge */}
