...
function CoverPrint({ cover, photos, W, H, accent }: { cover: any; photos: PhotoMeta[]; W: number; H: number; accent: string }) {
  const spiralH = Math.round(H * 0.045);
  const bgPhoto = cover?.photoId ? photos.find(p => p.id === cover.photoId) : null;
  return (
    <div data-print-page style={{ width: W, height: H, position: 'relative', background: cover?.bgColor || '#fff', flexShrink: 0, overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: spiralH, background: '#f1f5f9', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: Math.round(W / 14) }}>
        {Array.from({ length: 14 }).map((_, i) => (
          <div key={i} style={{ width: Math.round(W * 0.038), height: Math.round(W * 0.038), borderRadius: '50%', border: '1.5px solid #94a3b8', background: '#fff' }} />
        ))}
      ...
      {bgPhoto?.preview && (
        <img src={bgPhoto.preview} alt="" crossOrigin="anonymous"
          style={{ position: 'absolute', top: spiralH, left: 0, width: W, height: H - spiralH, objectFit: 'cover' }} />
      )}
      <div style={{ position: 'absolute', bottom: Math.round(H * 0.08), left: 0, right: 0, textAlign: 'center', color: cover?.titleColor || accent, fontWeight: 900, fontSize: Math.round(W * 0.07), textShadow: bgPhoto ? '0 2px 12px rgba(0,0,0,0.5)' : 'none' }}>
        {cover?.title || ''}
      </div>
    </div>
  );
...
export default function CalendarPrintPage({
  ...
  const marks = markedDates[`${page.year}-${page.month}`] || [];
