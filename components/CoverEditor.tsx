          ...
          {config.decoType === 'acryl' && (
            <div
              ...
              {photo ? <><div style={{ width:'100%', height:'100%', overflow:'hidden', position:'relative', cursor:'grab' }}
                    ...
                    <img src={photo.preview} style={{ width:`${Math.max(100,(config.photoZoom??1)*100)}%`, height:`${Math.max(100,(config.photoZoom??1)*100)}%`, objectFit:'cover', objectPosition:`${config.photoCropX??50}% ${config.photoCropY??50}%`, position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', userSelect:'none', pointerEvents:'none', touchAction:'manipulation' }} draggable={false}/>
                  </div>
